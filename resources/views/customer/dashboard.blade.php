...
.cust-dash-hero-top { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px; }
.cust-dash-brand { display:flex; align-items:center; gap:8px; }
.cust-dash-brand-icon { width:36px; height:36px; border-radius:10px; background:#fff; display:flex; align-items:center; justify-content:center; }
...
.cust-dash-readonly { background:rgba(255,255,255,.15); border-radius:20px; padding:4px 10px; display:flex; align-items:center; gap:4px; font-size:10px; color:#90CAF9; }
.cust-dash-title   { font-size:19px; font-weight:600; color:#fff; margin-bottom:2px; }
.cust-dash-company { font-size:13px; color:#90CAF9; margin-bottom:12px; }
.cust-dash-status  { display:inline-flex; align-items:center; gap:6px; background:rgba(105,240,174,.15); border:1px solid rgba(105,240,174,.4); border-radius:20px; padding:5px 14px; }
...
.cust-summary-strip { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:20px; }
@media (min-width: 640px) { .cust-summary-strip { grid-template-columns:1fr 1fr 1fr 1fr; } }
.cust-summary-card { background:var(--white); border:1px solid var(--border-light); border-radius:var(--radius-md); padding:10px; text-align:center; }
...
.cust-sec-row   { display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; }
.cust-sec-title { font-size:13px; font-weight:600; color:var(--text); }
.cust-sec-count { background:var(--blue-light); color:#0C447C; border-radius:20px; padding:3px 10px; font-size:11px; font-weight:600; }
...
.cust-report-item-meta { display:flex; flex-direction:column; gap:4px; margin-bottom:10px; }
...
.cust-report-photo { width:48px; height:48px; border-radius:var(--radius-md); object-fit:cover; flex-shrink:0; }
.cust-report-photo-placeholder { width:48px; height:48px; border-radius:var(--radius-md); background:var(--blue-light); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
...
.cust-info-card { background:var(--white); border:1px solid var(--border-light); border-radius:var(--radius-lg); overflow:hidden; margin-bottom:16px; }
.cust-info-card-header { background:var(--blue-light); padding:9px 14px; display:flex; align-items:center; gap:6px; font-size:12px; font-weight:600; color:#0C447C; }
.cust-info-card-header i { font-size:14px; color:var(--blue); }
.cust-info-grid { display:grid; grid-template-columns:1fr 1fr; gap:10px; padding:14px; }
        ...
        <div class="cust-report-item-head">
            <div style="display:flex;align-items:flex-start;gap:10px;flex:1;min-width:0;">
                @if(!empty($report->photos))
                    <img src="{{ asset('storage/' . $report->photos[0]) }}" alt="Foto" class="cust-report-photo">
                @else
