...
@section('css')
<style>
.sig-setup-wrap { max-width:520px; margin:3rem auto 0; }
.sig-setup-card { background:var(--white); border:1px solid var(--border); border-radius:var(--radius-lg); overflow:hidden; }
.sig-setup-header { padding:14px 20px; border-bottom:1px solid var(--border-light); background:var(--bg); display:flex; align-items:center; gap:8px; font-size:.82rem; font-weight:700; color:var(--text); }
.sig-setup-body { padding:1.5rem 1.25rem; }
.sig-setup-info { font-size:.84rem; color:var(--text-secondary); margin-bottom:1.5rem; line-height:1.6; }
@media (min-width: 640px) { .sig-setup-body { padding:2rem 1.75rem; } }
</style>
@endsection
...
<div class="sig-setup-wrap">
    <div class="sig-setup-card">
        ...
        <div class="sig-setup-body">
            ...
            <form action="{{ route('customer.profile.signature.store') }}" method="POST" id="sigForm">
                @csrf
...
$(function () {
    ...
    @php $sig = Auth::guard('customer')->user()->signature; @endphp
    @if($sig)
    (function() {
        var img = new Image();
        img.onload = function() {
        ...
        img.src = @json(asset('storage/' . $sig));
    })();
    @endif
    ...
    canvas.addEventListener('mousedown', function(e) { e.preventDefault(); drawing = true; var p = getPos(e); ctx.beginPath(); ctx.moveTo(p.x, p.y); if (!hasSig) markSigned(); });
    canvas.addEventListener('mousemove', function(e) { if (!drawing) return; e.preventDefault(); var p = getPos(e); ctx.lineTo(p.x, p.y); ctx.stroke(); });
